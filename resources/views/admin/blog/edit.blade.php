    ...
    <div data-kt-swapper="true" data-kt-swapper-mode="prepend" data-kt-swapper-parent="{default: '#kt_content_container', 'lg': '#kt_header_nav'}" class="page-title d-flex align-items-center flex-wrap me-3 mb-5 mb-lg-0">
        ...
        <ul class="breadcrumb breadcrumb-separatorless fw-bold fs-7 my-1">
            <li class="breadcrumb-item text-muted px-2">
                <a  href="{{route($route. '.index')}}" class="text-muted text-hover-primary">المدونات والأخبار</a>
            </li>
...
<script>
    $('.newstype').on('change', function() {
        if ($(this).find(":checked").val() == 'news') {
            $('.show-news').show()
            $('.show-video').hide()
        } else if ($(this).find(":checked").val() == 'image') {
            $('.show-news').hide()
            $('.show-video').hide()
        } else if ($(this).find(":checked").val() == 'video') {
            $('.show-news').hide()
            $('.show-video').show()
        }
    });
